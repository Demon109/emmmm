...
# 创建解析器, 将命令行解析成 Python 数据类型所需的全部信息。
parser = argparse.ArgumentParser()
parser.add_argument('--model_path', type=str, default='models_save')
parser.add_argument('--img_path', type=str, default='dataset/test.jpg')
...
# 画出人脸框和关键点
def draw_face(img_path, boxes_c, landmarks):
...
if __name__ == '__main__':
    # 预测图片获取人脸的box和关键点
    boxes_c, landmarks = infer_image(args.img_path)
    # 把关键画出来
    if boxes_c is not None:
        draw_face(img_path=args.img_path, boxes_c=boxes_c, landmarks=landmarks)
    else:
