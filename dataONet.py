...
def crop_28_box_image(data_path, filename, min_face_size, scale_factor, p_thresh, r_thresh):
    # pos，part,neg裁剪图片放置位置
    pos_save_dir = os.path.join(data_path, '28/positive')
    part_save_dir = os.path.join(data_path, '28/part')
    neg_save_dir = os.path.join(data_path, '28/negative')
    # RNet数据地址
    save_dir = os.path.join(data_path, '28/')
    ...
    save_hard_example(data_path, 28)
...
if __name__ == '__main__':
    ...
    # 获取人脸的box图片数据
    print('开始生成bbox图像数据')
    crop_28_box_image(data_path, filename, min_face_size, scale_factor, p_thresh, r_thresh)
    # 获取人脸关键点的数据
    ...
    crop_landmark_image(data_path, data_list, 28, argument=True)
    # 合并数据列表
    print('开始合成数据列表')
    combine_data_list(os.path.join(data_path, '28'))
    # 合并图像数据
    print('开始合成图像文件')
    convert_data(os.path.join(data_path, '28'), os.path.join(data_path, '28', 'all_data'))
    # 删除旧数据
    print('开始删除就得图像文件')
    delete_old_img(data_path, 28)
